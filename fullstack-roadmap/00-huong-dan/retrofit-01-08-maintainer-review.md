...
| R04 | High | PR patch Module 02–06 trước đó chỉ có 2–4 vấn đề độc lập ở mỗi patch. | Người học khó luyện phân loại issue và review theo nhiều nhóm. | Mở rộng từng patch thành ít nhất 6 root cause độc lập, bổ sung contract và rubric xét correctness/performance/security/maintainability/operability. Không buộc bịa finding ở nhóm không có lỗi. |
| R05 | Medium | Nhiều câu trong Failure Lab/Spaced Review/PR Lab dính số và từ, ví dụ `20file`, `Retrieval10điểm`, `Top2`. | Khó đọc và tăng tải nhận thức không cần thiết. | Biên tập thủ công các câu đã phát hiện; giữ identifier và baseline. Kiểm lại bằng tìm kiếm ngoài code fence/inline code. |
...
Module 02–08: mới đối chiếu tiêu đề, ví dụ nhỏ và các bước walkthrough để tìm mâu thuẫn rõ; chưa đọc sâu toàn bộ phần cơ chế, bài tập và prerequisite từng bài. Cần tiếp tục từ Module 02, ghi evidence cùng finding theo bài khi có lỗi thực tế.
